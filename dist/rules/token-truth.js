...
export const expectedInputDelta = (facts) => facts.tokenizer === "claude-v2" ? DELTA_NEW : DELTA_OLD;
async function tokenTruthResult(ctx) {
    const noCounts = {
    ...
    const outputs = [s?.completion ?? null, l?.completion ?? null].filter((v) => v !== null);
    checks.push({
        id: "output-bounded",
