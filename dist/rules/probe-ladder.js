...
const QUORUM = 3;
const labelsWith = (probes, sig) => probes
...
export const foreignRule = defineRule({
    ...
    judge: async (ctx) => {
        ...
        const onIdentity = probes.some((r) => r.signal === "foreign" &&
...
export const quorumRule = defineRule({
    ...
    judge: async (ctx) => {
        ...
        if (probes.length - failed.length >= QUORUM)
            return null;
