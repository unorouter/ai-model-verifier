 ...
 */
import { COUNT_PROBE_MAX_TOKENS, LONG_PROMPT, SHORT_PROMPT, } from "../internal/fixed-text";
/**
 ...
 */
export const DEFAULT_TIER_SIGNATURES = { haiku: [90] };
const intOf = (v) => typeof v === "number" && Number.isInteger(v) && v >= 0 ? v : null;
function readInputTokens(data, wire) {
    const u = data?.usage;
...
export async function measureTokenizerFingerprint(opts) {
    ...
    const url = opts.wire === "anthropic"
        ? `${base}/v1/messages`
        : `${base}/v1/chat/completions`;
    const headers = opts.wire === "anthropic"
        ? {
