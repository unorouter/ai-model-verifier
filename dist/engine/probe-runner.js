...
import { PROBES, } from "../probes/table";
...
const cap = (text) => text.length > RESPONSE_CAP ? `${text.slice(0, RESPONSE_CAP)}...` : text;
...
async function runProbe(ctx, probe) {
    ...
    for (let attempt = 0; attempt <= NONCE_MISMATCH_RETRIES; attempt++) {
        if (attempt > 0)
            await sleep(NONCE_MISMATCH_BACKOFF_MS);
        ...
        const log = (pass, signal, responseText, error) => ctx.onProbe?.({
            label: probe.label,
            attempt,
            pass,
            signal,
        ...
        if (!echoesNonce(text, nonce)) {
            log(false, null, text, `nonce_mismatch (attempt ${attempt + 1}/${NONCE_MISMATCH_RETRIES + 1}): expected "${nonce}"`);
...
export function collectProbes(ctx) {
...
export function richestProbe(probes) {
    return probes.reduce((best, r) => (r.usage?.completion ?? 0) > (best?.usage?.completion ?? 0) ? r : best, null);
}
