...
async function runProbe(args) {
    ...
    for (let attempt = 0; attempt <= NONCE_MISMATCH_RETRIES; attempt++) {
        ...
        if (res.error !== null) {
            const transient = res.corsBlocked ||
        ...
        const signal = detectSignal(text, probe.label, args.cfg.foreignIdentityPatterns, args.cfg.cloudModelNamePatterns, args.cfg.acceptsCloudHostIdentity);
...
export async function runVerification(opts) {
    ...
    const results = await Promise.all(PROBES.map((probe) => runProbe({
        probe,
        cfg,
        baseUrl: opts.baseUrl,
        apiKey: opts.apiKey,
