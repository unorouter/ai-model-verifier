...
export function sampleThroughput(args: {
  ...
  if (typeof tokens !== "number" || tokens < 20) return null;
  if (!Number.isFinite(args.elapsedMs) || args.elapsedMs <= 0) return null;
  return {
    lane: args.lane,
    model: args.model,
    outputTokens: tokens,
...
export function compareThroughput(
  ...
  opts?: { outlierRatio?: number },
): ThroughputComparison[] {
  const ratio = opts?.outlierRatio ?? 2;
  const byModel = new Map<string, ThroughputSample[]>();
  ...
  const out: ThroughputComparison[] = [];
      ...
      group.length >= 3
        ? group
            ...
            .map((s) => ({
              ...
              direction:
                s.tokensPerSecond > med ? ("faster" as const) : ("slower" as const),
            }))
