...
export const DEFAULT_ALWAYS_THINKS: readonly string[] = ["gemini-2-5-pro"];
...
export function mustAlwaysThink(
  ...
  return (options?.models ?? DEFAULT_ALWAYS_THINKS).some((p) =>
    m.startsWith(normalize(p)),
  );
}
...
export function readThinkingUsage(
  data: unknown,
): { completion: number | null; reasoning: number | null } | null {
  if (!data || typeof data !== "object") return null;
  const d = data as { usage?: unknown; usageMetadata?: unknown };
  const u = (d.usage ?? d.usageMetadata) as UsageShape | undefined;
  if (!u || typeof u !== "object") return null;
  const details = u.completion_tokens_details;
  const reasoning =
    num(details && typeof details === "object" ? details.reasoning_tokens : null) ??
...
export function checkThinkingFloor(
  model: string,
  data: unknown,
  options?: ThinkingFloorOptions,
): ThinkingFloorResult {
...
export async function probeThinkingFloor(opts: {
  ...
  const req: { url: string; headers: Record<string, string>; reqBody: unknown } =
