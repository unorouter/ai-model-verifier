...
import {
  COUNT_PROBE_MAX_TOKENS as MAX_TOKENS,
  LONG_PROMPT,
  SHORT_PROMPT,
...
export function expectedInputDelta(model: string): { min: number; max: number } {
...
function usageOf(data: unknown): { input: number | null; output: number | null } {
...
export async function checkTokenTruth(opts: {
  ...
  const outputs = [s.output, l.output].filter((v): v is number => v !== null);
  ...
  if (countRes.status !== null && countRes.status < 400) {
    counted = intOf((countRes.data as { input_tokens?: unknown })?.input_tokens);
    if (counted !== null) {
