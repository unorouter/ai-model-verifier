...
export type CompareResult = {
  jsd: number | null;
  cells: { label: string; jsd: number; samples: number; refSamples: number }[];
...
export function mergeFingerprints(
  ...
  for (const label of new Set([...Object.keys(a.cells), ...Object.keys(b.cells)])) {
    ...
    const answers: Record<string, number> = { ...x.answers };
...
export function jensenShannon(
  ...
  for (const k of new Set([...Object.keys(p), ...Object.keys(q)])) {
    ...
    const b = (q[k] ?? 0) / nq;
    const m = (a + b) / 2;
    if (a > 0) d += (a / 2) * log2(a / m);
    if (b > 0) d += (b / 2) * log2(b / m);
  }
...
export function compareFingerprints(
  ...
  if (cells.length < minCells) return { jsd: null, cells, verdict: "insufficient" };
  const jsd = cells.reduce((a, c) => a + c.jsd, 0) / cells.length;
  const verdict: CompareVerdict =
    jsd <= (opts.matchBits ?? DEFAULT_MATCH_BITS)
      ? "match"
...
export function compareToProfiles(
  sample: FingerprintSample,
  profiles: readonly { name: string; sample: FingerprintSample }[],
  opts: CompareOptions = {},
): { best: string | null; jsd: number | null; verdict: ProfileVerdict } {
  // The closest profile decides: a match to any is known, a mismatch to the
  // closest is a mismatch to all.
  let best: { name: string; r: CompareResult; jsd: number } | null = null;
  for (const p of profiles) {
    const r = compareFingerprints(sample, p.sample, opts);
    if (r.jsd === null) continue;
    if (!best || r.jsd < best.jsd) best = { name: p.name, r, jsd: r.jsd };
  }
  if (!best) return { best: null, jsd: null, verdict: "insufficient" };
  const verdict: ProfileVerdict =
    best.r.verdict === "match"
      ? "known"
...
export function fingerprintClusters(
  samples: Record<string, FingerprintSample>,
  threshold = DEFAULT_MATCH_BITS,
  opts: CompareOptions = {},
): { members: string[]; pooled: FingerprintSample }[] {
  const keys = Object.keys(samples);
  const clusters: { members: string[]; pooled: FingerprintSample }[] = [];
  for (const k of keys) {
    const s = samples[k]!;
    let placed = false;
    ...
    if (!placed) clusters.push({ members: [k], pooled: s });
  }
