...
export const PROBES = [
  ...
  },
  {
    ...
    grade: (text: string, identity: VendorIdentity) => {
      if (hasCodingToolRefusal(text) || hasScamPage(text)) return false;
      if (hasForeignIdentity(text, identity, "identity")) return false;
      if (includesAny(text, identity.home)) return true;
      return (
        identity.acceptsCloudHost && includesAny(text, CLOUD_HOST_PATTERNS)
      );
    },
  },
  {
    ...
    grade: (text: string, identity: VendorIdentity) => {
      if (hasCodingToolRefusal(text) || hasScamPage(text)) return false;
      if (hasForeignIdentity(text, identity, "model-name")) return false;
      if (!includesAny(text, identity.homeModelNames)) return false;
