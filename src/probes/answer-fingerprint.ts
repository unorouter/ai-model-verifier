...
const firstWord = (text: string): string | null => words(text)[0] ?? null;

const firstNumber = (text: string, min: number, max: number): string | null => {
  for (const w of words(text)) {
    const n = /^\d+$/.test(w) ? Number(w) : NUMBER_WORDS[w] ? Number(NUMBER_WORDS[w]) : null;
    if (n !== null && n >= min && n <= max) return String(n);
  }
  return null;
};
...
const oneOf =
  ...
  (text: string): string | null => {
    for (const w of words(text)) if (allowed.includes(w)) return w;
    return null;
  };
...
export const FINGERPRINT_CELLS = [
  {
    ...
    normalize: (t: string) => firstNumber(t, 1, 100),
  },
  {
    label: "colour",
  ...
  },
  {
    ...
    normalize: (t: string) => {
      for (const w of words(t)) if (/^[a-z]$/.test(w)) return w;
      const f = firstWord(t);
  ...
  },
  {
    label: "favourite",
    prompt:
      "What is your favourite number from 1 to 10? Reply with the number only.",
    normalize: (t: string) => firstNumber(t, 1, 10),
  },
  {
    label: "fruit",
...
] as const satisfies readonly FingerprintCell[];
